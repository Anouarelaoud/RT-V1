...
/*   Updated: 2020/01/26 20:15:21 by aait-el-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		initialize_raytracer(t_raytracer *r)
{
	...
	r->plane_vectors[1] = vecnorm(veccross(r->plane_vectors[0],
				vecsub(r->scene.camera->vectors[1],
	...
	if (TLIST(r->scene.objects, t_object)->object_type == SPHERE)
		...
		TLIST(r->scene.objects, t_object)->vectors[0] =
			vecadd(TLIST(r->scene.objects, t_object)->vectors[0],
					TLIST(r->scene.objects, t_object)->vectors[2]);
	rotate_direction(r);
}
...
static void		ray_constructor(t_raytracer *r, int plane_indexes[2])
{
	...
	r->ray.dir = vecnorm(vecadd(vecadd(vecsub(r->scene.camera->vectors[1],
						r->scene.camera->vectors[0]),
				vecopx(r->plane_vectors[X], scalars[X])),
...
void			render_scene(t_raytracer *raytracer)
{
	...
	while (++plane_indexes[X] < WIDTH)
	{
		...
		while (++plane_indexes[Y] < HEIGHT)
		{
			...
			if (hit_loop(raytracer, NULL))
				raytracer->image_data[plane_indexes[Y] * WIDTH +
					plane_indexes[X]] = color_picker(raytracer);
		}
	}
	mlx_put_image_to_window(raytracer->mlx_pointers[0],
			raytracer->mlx_pointers[1], raytracer->mlx_pointers[2], 0, 0);
}
